...
export default function Cart() {
  ...
  const api = `https://reasapiv2.azurewebsites.net/api/Auction`;
    ...
    axios
      ...
      .then((response) => {
        if (response.data && Array.isArray(response.data.data.pagingData)) {
          setSessionData(response.data.data.pagingData);
        } else {
    ...
    <Box key={session.sessionId}>
      ...
      >
        ...
        <Box display="flex" flexDirection={"column"}>
          ...
          {!matches && (
            <Typography variant="subtitle2">{session.description}</Typography>
          )}
        </Box>
        <Typography variant="body1" justifycontent={"end"}>
    {session.finalPrice?.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}
</Typography>
    ...
    >
      {sessionData.length > 0 ? (
        ...
        >
          ...
          <Typography variant="body1" align="center" color={Colors.muted}>
            Xin vui lòng thanh toán trong vòng 3 ngày nếu không bạn sẽ mất tiền
            cọc theo quy định của chúng tôi.{" "}
          </Typography>
          <Paper
            elevation={0}
            sx={{
          ...
          <Button sx={{ mt: 4 }} variant="contained" href="/shoppingcart">
            {" "}
            Thanh toán Ngay
          </Button>
        </Box>
        ...
        <Box
          ...
          alignItems="center"
        >
          <Typography variant={matches ? "h5" : "h3"} color={"black"}>
            Không Có Sản Phẩm
          </Typography>
        </Box>
      )}
